...
@section('content')
    <form method="post" action="/admin/posts/edit/{{$basePosts->slug}}" id="add-posts-form"
          class="form-horizontal form-label-left" enctype="multipart/form-data">
        <div class="row">
            <div class="col-md-9 col-sm-9 col-xs-12">
                {{csrf_field()}}
                <input type="hidden" name="count" value="{{$count}}"/>
                <input type="hidden" name="base_image" value="{{$basePosts->image}}"/>
                <div class="form-group">
                    <div class="row">
                        <div class="col-md-4 col-sm-4 col-xs-4">
                            <select name="category" class="form-control">
                                @foreach($categories as $category)
                                    @php $select = ($basePosts->category==$category->id)?'selected':''; @endphp
                ...
                <div class="form-group">
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <textarea name="title" id="ckeditor2">{{$basePosts->title}}</textarea>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <input type="text" name="short_title" class="form-control" value="{{$basePosts->short_title}}"/>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <textarea name="content" cols="30" rows="10" id="ckeditor1">{{$basePosts->content}}</textarea>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="row">
                        <div class="col-md-3 col-sm-3 col-xs-3">
                            <input type="text" name="date" class="form-control" id="date" value="{{$basePosts->date}}"/>
                        </div>
                        <div class="col-md-6 col-sm-6 col-xs-6">
                            <select class="select2_multiple form-control" name="positions[]" multiple="multiple">
                                @foreach($positions as $position)
                                    @php $select = (in_array($position->id, $basePostsPositions))?'selected':''; @endphp
                                    <option value="{{$position->id}}" {{$select}}>{{$position->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <div class="col-md-3 col-sm-3 col-xs-12">
                <div class="form-group">
                    ...
                    <div id="img-base-upload" class="col-md-12 col-sm-12 col-xs-12 mb-2">
                        @php
                            $image_base_name = explode('/', str_replace('\\', '/', $basePosts->image));
                            $image_base_name = last($image_base_name);
                        @endphp
                        <div class="row">
                            <div class="col-md-6 col-sm-6 col-xs-6 col-md-offset-3 p-0">
                                <img src="{{\App\Http\Helpers::getImageUrl($basePosts->image, 'small')}}" class="img-thumbnail"/>
                            </div>
                        </div>
                    </div>
                    <div class="form-group input-file-container">
                    ...
                    <div id="img-upload" class="col-md-12 col-sm-12 col-xs-12">
                        @foreach($images as $image)
                            @php
                                $image_name = explode('/', str_replace('\\', '/', $image['url']));
                                $image_name = last($image_name);
                            @endphp
                            <div class="row">
                                <div class="col-md-6 col-sm-6 col-xs-6 col-md-offset-3 p-0">
                                    <img src="{{\App\Http\Helpers::getImageUrl($image['url'], 'small')}}" class="img-thumbnail"/>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="form-group">
                    <div class="row pt-2 pb-2" style="padding: 10px 0px;">
                        <div class="col-md-12 col-sm-12 col-xs-12 mt-2">
                            <textarea name="tags" cols="30" rows="5" id="tags" placeholder="Tags">{{$basePosts->tags}}</textarea>
                        </div>
                    </div>
                    <div class="row pt-2 pb-2">
                        <div class="col-md-12 col-sm-12 col-xs-12 mt-2">
                            <textarea name="description" class="form-control" rows="5"
                                      placeholder="Description">{{$basePosts->description}}</textarea>
                        </div>
                    </div>
                </div>
            </div>
