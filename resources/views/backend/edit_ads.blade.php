...
@section('content')
    <form method="post" action="/admin/ads/edit/{{$baseAds->id}}" id="add-ads-form" class="form-horizontal form-label-left">
        {{csrf_field()}}
        <div class="form-group">
            <label class="control-label col-md-3 col-sm-3 col-xs-12">Position</label>
            <div class="col-md-9 col-sm-9 col-xs-12">
                <select name="position" class="form-control">
                    <option value="home_center" @if($baseAds->position == 'home_center') selected @endif >home_center</option>
                    <option value="home_right" @if($baseAds->position == 'home_right') selected @endif >home_right</option>
                    <option value="post_details" @if($baseAds->position == 'post_details') selected @endif >post_details</option>
                </select>
            </div>
        </div>
        <div class="form-group">
            <div class="row">
                ...
                <div class="col-md-9 col-sm-9 col-xs-12">
                        <textarea name="script_text" cols="30" rows="10" class="ckeditor">{{ $baseAds->script_text }}</textarea>
                </div>
            </div>
        </div>
